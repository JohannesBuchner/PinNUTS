...
class PinNUTSSampler(Sampler):
    ...
    def __init__(self, dim, lnprobfn, gradfn=None, *args, **kwargs):
            self.dim = dim
            self.f = PinNutsSampler_fn_wrapper(lnprobfn, gradfn, *args, **kwargs)
    ...
    def sample(self, pos0, M, Madapt, delta=0.6, **kwargs):
        """ Runs NUTS6 """
        samples, lnprob, epsilon = pinnuts(self._sample_fn, M, Madapt, pos0, delta)
        self._chain = samples
        self._lnprob = lnprob
...
def test_sampler():
    """ Example usage of PinNUTS_sampler: sampling a 2d highly correlated Gaussian distribution """
